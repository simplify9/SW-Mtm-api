# mtm (SW-Mtm API) Helm chart

Deploys the SW-Mtm API as a Kubernetes `Deployment` + `Service` (plus a `Secret`
for the DB connection string and token settings), exposed north-south through
...
```sh
helm install mtm ./charts/default \
  --set ingress.hosts[0]=mtm.example.com
...
helm install mtm ./charts/default \
  ...
  --set gateway.hostnames[0]=mtm.example.com \
